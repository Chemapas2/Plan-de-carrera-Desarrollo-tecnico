...
def build_final_report_text(analysis, selected_species, selected_subspecies, base_internal, base_external, adv_internal, adv_external, qa_items):
    ...
    lines.extend(render_course_lines(base_internal, "7.1 Formación base interna"))
    lines.extend(render_course_lines(base_external, "7.2 Formación base externa"))
    lines.extend(render_course_lines(adv_internal, "7.3 Formación especializada interna"))
    lines.extend(render_course_lines(adv_external, "7.4 Formación especializada externa"))
    lines.append("")
    lines.append("## 8. Gráficos de araña del assessment")
    ...
    if included_qas:
        lines.append("")
        lines.append("## 9. Preguntas y respuestas incorporadas")
...
ASSET_CANDIDATES = {
    "catedra": ["Catedra Nanta Zaragoza.jpg"],
...
def display_corporate_header(asset_paths):
    ...
    if asset_paths.get("catedra"):
        with c1:
            st.image(str(asset_paths["catedra"]), use_container_width=True)
    with c2:
        st.markdown(
            """
            <div style="padding-top:8px;">
              ...
              <div style="margin-top:8px;color:#6b7280;font-size:15px;line-height:1.45;">
                Entorno corporativo TechTeam · Nutreco · Cátedra Nanta de Ganadería de Precisión.
              </div>
            </div>
            """,
    ...
    if asset_paths.get("nutreco"):
        st.image(str(asset_paths["nutreco"]), width=240)
...
def make_html_report_bytes(report_text, analysis, selected_species, selected_subspecies, chart_images=None, asset_paths=None):
    ...
    logos = {k: image_to_data_uri(v) for k, v in asset_paths.items() if k in {"catedra", "nutreco", "techteam", "strip"}}
...
.hero-grid {{ display:grid; grid-template-columns:220px 170px 250px 1fr; gap:14px; align-items:center; }}
...
<div class='container'>
  <div class='hero'>
    <div class='hero-grid'>
      <img src='{logos.get("catedra", "")}' alt='Cátedra Nanta'>
...
display_corporate_header(asset_paths)
st.caption(
    "Carga la evaluación de un técnico, interpreta el assessment y genera un informe editable con comparativas, "
    "prioridades de desarrollo, recomendaciones formativas internas y externas, y acceso directo al buscador global de formación."
)

assessment_tab, finder_tab = st.tabs(["Plan de carrera", "Buscador global de formación"])
...
with assessment_tab:
    ...
    with sidebar.expander("Clasificación del perfil", expanded=False):
        st.caption(
            "La categoría se toma del propio Excel y debe coincidir con ENLACES DATOS!N44. "
            "La lógica usa ENLACES DATOS!K37 como ratio de clasificación y los cortes de ENLACES DATOS!M56:M61."
        )
    ...
    if not assessment_file:
        st.info("Sube un archivo de assessment para activar el análisis y las exportaciones del plan de carrera.")
    else:
        try:
        ...
        if analysis.get("profile_ratio") is not None:
            st.caption(
                f"Clasificación según Excel: {analysis['profile']} · ratio de clasificación = {analysis['profile_ratio']:.2f} · ranking = {analysis.get('profile_rank', 'No disponible')}"
            )
        else:
            st.caption(f"Clasificación según Excel: {analysis['profile']}")
        st.markdown(build_executive_report(analysis, selected_species, selected_subspecies))
        if analysis.get("profile_thresholds"):
            threshold_df = pd.DataFrame(
                [
                    {
                        "Ranking Excel": item["rank"],
                        "Etiqueta": item["label"],
                        "Corte (> )": item["cutoff"],
                    }
        ...
        auto_report = build_final_report_text(analysis, selected_species, selected_subspecies, base_internal, base_external, adv_internal, adv_external, st.session_state["qa_items"])
        ...
        if st.button("Refrescar informe con el contenido actual"):
            st.session_state["final_report_text"] = build_final_report_text(analysis, selected_species, selected_subspecies, base_internal, base_external, adv_internal, adv_external, st.session_state["qa_items"])
            st.rerun()
        ...
        st.info("En la pestaña ‘Buscador global de formación’ tienes acceso directo al buscador avanzado corporativo. Puedes usarlo sin salir de esta app.")
