...
st.caption(
    ...
    "prioridades de desarrollo, recomendaciones formativas internas y externas, y preguntas-respuestas incorporables al informe."
)
...
docx_bytes = make_docx_bytes(final_report_text, chart_images=chart_images)
pdf_bytes = make_pdf_bytes(final_report_text, chart_images=chart_images)
...
with d2:
    st.download_button("Descargar informe en DOCX", data=docx_bytes, file_name="informe_plan_carrera.docx", mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document", use_container_width=True)
with d3:
    st.download_button("Descargar informe en PDF", data=pdf_bytes, file_name="informe_plan_carrera.pdf", mime="application/pdf", use_container_width=True)
with d4:
