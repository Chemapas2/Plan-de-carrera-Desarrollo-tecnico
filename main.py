...
def make_docx_bytes(report_text):
    doc = Document()
...
for series in ["Técnico", "Referencia", "Benchmark aprox."]:
    fig_radar.add_trace(go.Scatterpolar(r=radar_df[series].tolist(), theta=radar_df["tronco"].tolist(), fill="toself", name=series))
...
docx_bytes = make_docx_bytes(final_report_text)
pdf_bytes = make_pdf_bytes(final_report_text)
